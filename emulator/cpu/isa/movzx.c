...
make_helper(movzbv)
{
	if (suffix == 'w')
	{
		...
		if (m.mod != 3)
		{
			...
			reg_w(m.reg) = (uint8_t)swaddr_read(mem_i, 1);
			print_asm("movzbw  " "%s,%%%s", ModR_M_asm, regsw[m.reg]);
			return 1 + len;
		}
		else
		{
			reg_w(m.reg) = (uint8_t)reg_b(m.R_M);
	...
	else
	{
		...
		{
			swaddr_t mem_i;  uint8_t len = read_ModR_M(eip+1, &mem_i);
			reg_l(m.reg) = (uint8_t)swaddr_read(mem_i, 1);
			print_asm("movzbl  " "%s,%%%s", ModR_M_asm, regsl[m.reg]);
			return 1 + len;
		}
		else
		{
			reg_l(m.reg) = (uint8_t)reg_b(m.R_M);
...
make_helper(movzwl)
{
	...
	{
		swaddr_t mem_i;  uint8_t len = read_ModR_M(eip+1, &mem_i);
		reg_l(m.reg) = (uint16_t)swaddr_read(mem_i, 2);
		print_asm("movzwl  " "%s,%%%s", ModR_M_asm, regsl[m.reg]);
		return 1 + len;
	}
	else
	{
		reg_l(m.reg) = (uint16_t)reg_w(m.R_M);
