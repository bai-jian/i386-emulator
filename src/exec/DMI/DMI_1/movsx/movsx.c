...
make_helper(movsbv)
{
	if (suffix == 'w')
	{
		...
		if (m.mod != 3)
		{
			...
			swaddr_write(mem_i, 2, (int8_t)reg_b(m.reg));
			print_asm("movsbw  " "%%%s,%s", regsb[m.reg], ModR_M_asm);
			return 1 + len;
		}
		else
		{
			reg_w(m.R_M) = (int8_t)reg_b(m.reg);
	...
	else
	{
		...
		{
			swaddr_t mem_i;  uint8_t len = read_ModR_M(eip+1, &mem_i);
			swaddr_write(mem_i, 4, (int8_t)reg_b(m.reg));
			print_asm("movsbl  " "%%%s,%s", regsb[m.reg], ModR_M_asm);
			return 1 + len;
		}
		else
		{
			reg_l(m.R_M) = (int8_t)reg_b(m.reg);
...
make_helper(movswl)
{
	...
	{
		swaddr_t mem_i;  uint8_t len = read_ModR_M(eip+1, &mem_i);
		swaddr_write(mem_i, 4, (int16_t)reg_w(m.reg));
		print_asm("movswl  " "%%%s,%s", regsw[m.reg], ModR_M_asm);
		return 1 + len;
	}
	else
	{
		reg_l(m.R_M) = (int16_t)reg_w(m.reg);
