...
make_helper( concat(cmp_i2rm_, SUFFIX) )
{
	...
	{ 
		...
		DATA_TYPE_S imm =  imm_byte == 1 ? (int8_t)instr_fetch(eip+1+len, 1) : instr_fetch(eip+1+len, imm_byte);
	...
	else
	{
		...
		DATA_TYPE_S imm =  imm_byte == 1 ? (int8_t)instr_fetch(eip+1+1, 1) : instr_fetch(eip+1+1, imm_byte);
