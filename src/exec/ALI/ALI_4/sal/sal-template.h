...
make_helper( concat(sal_, SUFFIX) )
{
	...
	else
	{
		...
		print_asm("sal" str(SUFFIX) "  %%%s", REG_NAME(reg_i));
...
make_helper( concat(sal_CL_, SUFFIX) )
{
	...
	{ 
		...
		print_asm("sal" str(SUFFIX) "  %%cl,%%%s", REG_NAME(reg_i));

		return 1 + 1;
	}
}

make_helper( concat(sal_ib_, SUFFIX) )
{
	ModR_M m;  m.val = instr_fetch(eip+1, 1);
	if (m.mod != 3)
	{
	...
	else
	{
		...
		print_asm("sal" str(SUFFIX) "  $0x%x,%%%s", imm, REG_NAME(reg_i));
